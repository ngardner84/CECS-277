...
public class GUI {
		...
		class SaveButtonListener implements ActionListener {
			...
			public void actionPerformed(ActionEvent click) {
				...
				if (c.equals("Save Order")) {
					DrinkItem coffeeDrink = new CoffeeItem(sizeOptions.getSelectedItem().toString(),
							sugarTsp.getSelectedItem().toString(), flavorOptions.getSelectedItem().toString(),
							baseOptions.getSelectedItem().toString(), temperatureOptions.getSelectedItem().toString(),
			...
			public void actionPerformed(ActionEvent click) {
				...
				if (c.equals("Save")) {
					...
					case "Cheesecake Slice": {
						...
						PastryItem cheeseCake = new PastryItem("Cheesecake Slice", cheeseCakeOption,
								heatedCheckBox.getState());
	...
	public static class NewTeaOrderFrame extends JFrame {
		...
		class SaveButtonListener implements ActionListener {
			...
			public void actionPerformed(ActionEvent click) {
				...
				if (c.equals("Save Order")) {
					TeaItem newTea = new TeaItem(sizeOptions.getSelectedItem().toString(),
							sweetnessOptions.getSelectedItem().toString(), flavorOptions.getSelectedItem().toString(),
							baseOptions.getSelectedItem().toString());
	...
	public static class TransactionFrame extends JFrame {
		...
		public TransactionFrame() {
			GUI.cash.setTotal();
			GUI.cash.totalWithTax = GUI.cash.totalAfterTax();
			System.out.println(GUI.cash.totalAfterTax());
			createComponents();
		...
		private void createComponents() {
			label = new JLabel("Amount Due: $" + df.format(GUI.cash.totalWithTax));
			receiptText = new JTextArea(30, 50);
			...
			receiptText.append("\n\n\tSubtotal:\t$" + df.format(GUI.cash.totalBeforeTax()) + "\n\tTax:\t $"
					+ df.format(GUI.cash.totalBeforeTax() * GUI.cash.tax) + "\n\tGrand Total:\t$"
					+ df.format(GUI.cash.totalAfterTax()));
		...
		class PayButtonListener implements ActionListener {
			...
			public void actionPerformed(ActionEvent click) {
				...
				if(textfield.getText().equals("")) {
					...
					payment = Double.parseDouble(textfield.getText().trim());
				}

				do {
					{
						if (GUI.cash.totalWithTax == payment) {
							label.setText("Thank you!");
							receiptText.append("\n\n\tThank you! Press exit to leave the screen");
							;
							break;
						} else if (payment > GUI.cash.totalWithTax) {
							GUI.cash.payment += payment;
							label.setText("Thank you!");
							receiptText.append("\n\n\tPayment: $" + df.format(GUI.cash.payment) + "\n\tChange Due: $" + GUI.cash.giveChange(payment)
									+ ".\n\t~~~~~~~~~~~~~~~~~~~~~~~~ THANK YOU! ~~~~~~~~~~~~~~~~~~~~~~~~~~~");
							...
							GUI.cash.payment += payment;
							GUI.cash.totalWithTax -= payment;
							label.setText("Insufficient payment! Amount still due: $" + df.format(GUI.cash.totalWithTax));
							break;
						}
					}
				} while (GUI.cash.totalWithTax > payment);
			}
		}
